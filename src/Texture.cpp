...
NAMESPACE {
  ...
  void Texture::load(String filename, const ShaderUniform* tex_uniform) {
    int width, height;
  ...
  void Texture::init(unsigned int num_textures) {
    glGenTextures(length, ids);
    Texture::length = num_textures;
