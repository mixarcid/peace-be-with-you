...
NAMESPACE {
  ...
  PhysicsComponent::PhysicsComponent(Renderable* rend,
				 Material mat,
				 Vec3f xi,
				 Vec3f vi,
				 PhysicsComponentFlags _flags)
    : Node(xi),
    material(mat),
    mass_data(0,0),
    veloc(vi),
  ...
  void PhysicsComponent::update(f32 dt) {
  ...
  void PhysicsComponent::applyForce(Vec3f f) {
    force += f;
  }
